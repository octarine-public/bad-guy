import { ArrayExtensions, Courier, DOTA_ABILITY_BEHAVIOR, Entity, EntityManager, GameSleeper, Hero, SpiritBear, Unit, Vector3 } from "github.com/octarine-public/wrapper/index"
...
function UseAbilites(unit: Unit, name: string, Position?: Vector3 | Unit): boolean {
	const abil = unit.GetAbilityByName(name)
...
function MoveUnit(unit: Unit) {
	...
	return Fountains.some(fnt => {
		if (fnt === undefined)
			return false
		const tp = unit.Inventory.TotalItems.find(item => item?.Name === "item_tpscroll")
		if (tp === undefined
...
export function Tick() {
	...
	Units = [
		...
		...EntityManager.GetEntitiesByClass(SpiritBear),
	]
